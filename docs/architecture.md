...
- `orders`: converted quote sales with project status, payment status, totals, dates, and a unique source `quoteId`.
...
10. The database enforces one order per quote through a unique constraint on `orders.quoteId`; duplicate conversion races reload the existing order when possible.
11. Payments update order `amountPaid` and payment status.
12. Analytics aggregate quotes, orders, revenue, conversion, salesperson metrics, and low-stock inventory.
